...
class Meta:
    def __init__(self, metadata: Optional[Dict] = None, filepath: Optional[str] = None):
        ...
        if filepath:
            print(f"Loading meta from {filepath}.")
    ...
    def _split_item_id(self):
        if "item_id" in self._metadata:
            self._metadata["item_id_prefix"] = [
                item_id.split(_ITEM_ID_PREFIX_DELIM)[0] if _ITEM_ID_PREFIX_DELIM in item_id else "" for item_id in
                self._metadata["item_id"]
            ]
            self._metadata["item_id"] = [
                item_id.split(_ITEM_ID_PREFIX_DELIM)[1] if _ITEM_ID_PREFIX_DELIM in item_id else item_id
                for item_id in self._metadata["item_id"]
            ]

    def _get_meta(self, key: str, out_type: Optional[Callable] = None) -> List[Union[int, str]]:
    ...
    def get_idx_to_id(self, key: str, out_type=None) -> Dict[int, Union[int, str]]:
        meta = self._get_meta(key, out_type)
    ...
    def get_meta_size(self, key: str) -> int:
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        output0_config = pb_utils.get_output_config_by_name(
            model_config, "bert4rec_preprocessing_output__0"
        )
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            inference_response = pb_utils.InferenceResponse(
                output_tensors=[input_sequence_tensor, target_idx_tensor]
            )
